codeunit 50128 "Waldo App Area Mgt"
{
    ...
    begin
        TempApplicationAreaSetup."Waldo App Area" := true;
    ...
    begin
        if ExperienceTierSetup.Essential then
            if not TempApplicationAreaSetup."Waldo App Area" then
    ...
    begin
        if AppAreaSetup.FindFirst() then
            exit(AppAreaSetup."Waldo App Area");
    end;
    ...
    begin
        ...
        if AppAreaSetup.FindFirst() then begin
            AppAreaSetup."Waldo App Area" := true;
            AppAreaSetup.Modify();
        end;
    end;
}